...
ChainTables exports no names. Everything is reached as `ChainTables.sync!(copy)`,
`ChainTables.commit!(w)`, `ChainTables.as_of(chain, 40)` — the precedent is
SQLite.jl, which exports only `DBInterface`, `SQLiteException` and `@sr_str`. The
names this
...
`ChainTables.open` reads as documentation at every call site.
...
`ChainTables.create_chain(chain)` is the only thing that may write slot 0. It mints
...
ChainTables.insert_rows!(w, :customers, rows)
ChainTables.commit!(w; comment = "…")   # → (; slot, transaction_hash, state_fingerprint)
```
...
an op, a duplicate key inside an op — so the stack trace points at the user's
own line. ADR-0001's data gate, *an op matching a row count other than the one
it names is an error*, runs at `commit!` against the model, where real state is
what it is checked against.

The four type tags are `Int64`, `Float64`, `String` and `Vector{UInt8}`, one per
value type (ADR-0025). `Int`, `Bool`, `DateTime` and `AbstractString` are refused
by name. This is the one place the project's "do not over-restrict argument
types" rule is deliberately inverted: a tag mapping to two value types, or to
none, is the divergence the design exists to prevent. Row sets are any Tables.jl
row table, because that is what a table view yields (ADR-0024).
...
`ChainTables.Testing.InMemoryObjectStore` ships in the package rather than living
in `test/`. A user testing their own ChainTables-backed code needs a store that never
...
- **`open` requires a path**; only `as_of` may take `path = nothing` for a
  temporary directory deleted on close. A live copy you cannot find again is a
  footgun, and a throwaway live copy is `as_of` at the head.
- **`open` creates the directory when it is absent**, unbound and fresh —
  ADR-0023 names that a legitimate open state, and binding happens at the
  first `sync!`.
- **The open-time warning** (ADR-0016's non-AWS store; this ADR first had a
  second, for a foreign database library, which left with ADR-0022) is `@warn`
  with `maxlog = 1` per process per condition, and `as_of` warns through the
  same path. The refusal at `commit!` never has `maxlog`: a suppressed warning
  must not become a suppressed refusal.
...
- **The record cache defaults to `first(DEPOT_PATH)/chaintables/records`**,
  overridable per chain and by `CHAINTABLES_CACHE_DIR`. Depot-based is
  machine-wide in ADR-0010's sense and needs no dependency, which `Scratch.jl`
  would have been. ADR-0010 carries the same default.
- **v1 documents one process and one thread per local copy** without implementing
  locking. ADR-0023's sweep deletes files a second process may be about to name
  in its own head, and two processes hold independent in-memory models; nothing
  makes a second process block rather than break. The locking policy itself
  stays open.
