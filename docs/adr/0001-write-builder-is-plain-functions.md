...
author's code is a different property, and ChainTables does not offer it.
...
- `missing` is accepted as null, because it is what a table view yields for a
  null cell (ADR-0024) and refusing it would break the read-change-write round
  trip. The accidental-null case it admits is caught by the shape's nullability
  (ADR-0025), which fails in the builder before a commit.
